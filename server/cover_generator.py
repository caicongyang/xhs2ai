...
class CoverStyle(str, Enum):
    ...
    SOFT_TECH = "soft_tech"  # 柔和科技卡片风
    BUSINESS = "business"  # 现代商务资讯卡片风
    TECH_BLUE = "tech_blue"  # 流动科技蓝风格
    MINIMALIST = "minimalist"  # 极简格栅主义封面风格
    DIGITAL_TICKET = "digital_ticket"  # 数字极简票券风
    CONSTRUCTIVISM = "constructivism"  # 新构成主义教学风
    LUXURY_NATURE = "luxury_nature"  # 奢华自然意境风
    INDUSTRIAL_PUNK = "industrial_punk"  # 新潮工业反叛风
    CUTE_KNOWLEDGE = "cute_knowledge"  # 软萌知识卡片风
    BUSINESS_CARD = "business_card"  # 商务简约信息卡片风

class CoverDesign(BaseModel):
    html_code: str = Field(description="完整的HTML代码")
    title: str = Field(description="封面标题")
...
class CoverGenerator:
    def __init__(self):
        ...
        # 预设风格
        self.styles = {
            CoverStyle.SOFT_TECH: self._get_soft_tech_style(),
            CoverStyle.BUSINESS: self._get_business_style(),
            CoverStyle.TECH_BLUE: self._get_tech_blue_style(),
            CoverStyle.MINIMALIST: self._get_minimalist_style(),
            CoverStyle.DIGITAL_TICKET: self._get_digital_ticket_style(),
            CoverStyle.CONSTRUCTIVISM: self._get_constructivism_style(),
            CoverStyle.LUXURY_NATURE: self._get_luxury_nature_style(),
            CoverStyle.INDUSTRIAL_PUNK: self._get_industrial_punk_style(),
            CoverStyle.CUTE_KNOWLEDGE: self._get_cute_knowledge_style(),
            CoverStyle.BUSINESS_CARD: self._get_business_card_style(),
        }
        
        self.wechat_prompt = ChatPromptTemplate.from_messages([
            ("system", """你是一位优秀的网页和营销视觉设计师，具有丰富的UI/UX设计经验，曾为众多知名品牌打造过引人注目的营销视觉，擅长将现代设计趋势与实用营销策略完美融合。
...
{style_instructions}"""),
        ...
        self.xhs_prompt = ChatPromptTemplate.from_messages([
            ("system", """你是一位优秀的网页和营销视觉设计师，具有丰富的UI/UX设计经验，曾为众多知名品牌打造过引人注目的营销视觉，擅长将现代设计趋势与实用营销策略完美融合。现在需要为我创建一张专业级小红书封面。请使用HTML、CSS和JavaScript代码实现以下要求：
...
{style_instructions}"""),
        ...
        self.wechat_chain = LLMChain(
            llm=self.llm,
            prompt=self.wechat_prompt
        )
        
        self.xhs_chain = LLMChain(
            llm=self.llm,
            prompt=self.xhs_prompt
        )
    
    def _get_soft_tech_style(self) -> str:
        """柔和科技卡片风格的提示词"""
        return """
## 风格要求：柔和科技卡片风
...
"""
    ...
    def _get_business_style(self) -> str:
        """现代商务资讯卡片风格的提示词"""
        return """
## 风格要求：现代商务资讯卡片风
...
"""
    ...
    def _get_tech_blue_style(self) -> str:
        """流动科技蓝风格的提示词"""
        return """
## 风格要求：流动科技蓝风格
...
"""
    ...
    def _get_minimalist_style(self) -> str:
        """极简格栅主义封面风格的提示词"""
        return """
## 风格要求：极简格栅主义封面风格
...
"""
    ...
    def _get_digital_ticket_style(self) -> str:
        """数字极简票券风格的提示词"""
        return """
## 风格要求：数字极简票券风
...
"""
    ...
    def _get_constructivism_style(self) -> str:
        """新构成主义教学风格的提示词"""
        return """
## 风格要求：新构成主义教学风
...
"""
    ...
    def _get_luxury_nature_style(self) -> str:
        """奢华自然意境风格的提示词"""
        return """
## 风格要求：奢华自然意境风
...
"""
    ...
    def _get_industrial_punk_style(self) -> str:
        """新潮工业反叛风格的提示词"""
        return """
## 风格要求：新潮工业反叛风
...
"""
    ...
    def _get_cute_knowledge_style(self) -> str:
        """软萌知识卡片风格的提示词"""
        return """
## 风格要求：软萌知识卡片风
...
"""
    ...
    def _get_business_card_style(self) -> str:
        """商务简约信息卡片风格的提示词"""
        return """
## 风格要求：商务简约信息卡片风
...
"""
    ...
    def generate_wechat_cover(self, title: str, emoji_url: Optional[str] = None, style: str = "default") -> str:
        """生成微信公众号封面HTML"""
        # 获取风格提示词
        style_instructions = ""
        if style and style != "default" and style in [s.value for s in CoverStyle]:
            style_enum = CoverStyle(style)
            style_instructions = self.styles.get(style_enum, "")
        
        # 使用LLM生成设计
        result = self.wechat_chain.invoke({
            "title": title,
            "emoji_url": emoji_url,
            "style": style,
        ...
        filename = f"templates/wechat_cover_{timestamp}_{style}.html"
        with open(filename, "w", encoding="utf-8") as f:
            f.write(result["text"])
        
        return filename
    
    def generate_xiaohongshu_cover(self, content: str, account_name: str, slogan: Optional[str] = None, style: str = "default") -> str:
        """生成小红书封面HTML"""
        # 获取风格提示词
        style_instructions = ""
        if style and style != "default" and style in [s.value for s in CoverStyle]:
        ...
        # 保存HTML文件
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"templates/xiaohongshu_cover_{timestamp}_{style}.html"
        with open(filename, "w", encoding="utf-8") as f:
            f.write(result["text"])
...
def main():
    ...
    这样非常方便在单个文件搜索内容 有任何想法或待办事项出现时，都会简单地以纯文本形式将它添加到笔记的顶部 
    只使用使用"watch:"（观看）、"listen:"（聆听）或"read:"（阅读）等标签 回顾的时候会不时向下滚动浏览笔记。
    如果发现任何值得持续关注的内容，会通过简单复制粘贴将其"救回"顶部。 
    值得记录的问题或者内容确实没有多少，这样很简单，项目类型的记录和文档他还是使用Obsidian"""
    account_name = "歸藏(guizang.ai)"
    slogan = "正儿八经学AI"
    xhs_file = generator.generate_xiaohongshu_cover(xhs_content, account_name, slogan)
    print(f"小红书封面已生成：{xhs_file}")
